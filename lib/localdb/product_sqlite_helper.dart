...
class ProductSQLiteHelper {
  ...
  Future<void> init() async {
    ...
    final dbPath = p.join(dir.path, 'products.db'); // Optional: separate file
...
void debugPrintAllProducts() {
  ...
  for (final row in result) {
    ...
    final price = row['price']?.toString().padRight(9) ?? '0.00'.padRight(9);
    final categoryId = row['categ_id']?.toString().padRight(11) ?? ''.padRight(11);
    final taxId = row['taxes_id']?.toString().padRight(10) ?? ''.padRight(10);
